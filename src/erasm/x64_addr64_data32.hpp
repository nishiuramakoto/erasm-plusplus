#ifndef ERASM_X64_ADDR64_DATA32_HPP
#define ERASM_X64_ADDR64_DATA32_HPP
/* DO NOT EDIT! 
...
#include "erasm/x64_prefix.hpp"
namespace erasm {
namespace x64 {
namespace addr64 {
namespace data32 {
/* RexW 15     id   | Add with carry imm32 sign extended to 64-bits to RAX. */
int adc (code_ptr p,
         const RegRAX (&   unused),
         imm32_t imm);
/* RexW 83  /2 RMBoth  ib   | Add with CF sign-extended imm8 into r/m64. */
int adc (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm8_t imm);
/* RexW 81  /2 RMBoth  id   | Add with CF imm32 sign extended to 64-bits to r/m64. */
int adc (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm32_t imm);
/* RexW 81  /2 RMBoth  id   | Add with CF imm32 sign extended to 64-bits to r/m64. */
...
int adc (code_ptr p,
         const WordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 13  /r RMBoth     | Add with CF r/m64 to r64. */
int adc (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 13  /r RMBoth     | Add with CF r/m64 to r64. */
int adc (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordPtr (&   modrm_rm));
/* RexW 11  /r RMBoth     | Add with CF r64 to r/m64. */
int adc (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const QwordReg (&   modrm_reg));
...
int adc (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const ByteReg86 (&   modrm_rm));
/*  12  /r RMBoth     | Add with carry r/m8 to byte register. */
int adc (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const BytePtr86 (&   modrm_rm));
/* Rex 12  /r RMBoth     | Add with carry r/m64 to byte register. */
int adc (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
/* Rex 12  /r RMBoth     | Add with carry r/m64 to byte register. */
int adc (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const BytePtrRex (&   modrm_rm));
/* Rex 12  /r RMBoth     | Add with carry r/m64 to byte register. */
int adc (code_ptr p,
         const ByteRegRex (&   modrm_reg),
         const ByteRegLow (&   modrm_rm));
/* Rex 12  /r RMBoth     | Add with carry r/m64 to byte register. */
int adc (code_ptr p,
         const ByteRegRex (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
...
int adc (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /2 RMBoth  ib   | Add with carry imm8 to r/m8. */
int adc (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /2 RMBoth  ib   | Add with carry imm8 to r/m8. */
int adc (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         imm8_t imm);
/* Rex 10  /r RMBoth     | Add with carry byte register to r/m64. */
int adc (code_ptr p,
         const BytePtr (&   modrm_rm),
         const ByteRegRex (&   modrm_reg));
...
int adc (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         const ByteRegLow (&   modrm_reg));
/* RexW 05     id   | Add imm32 sign-extended to 64-bits to RAX. */
int add (code_ptr p,
         const RegRAX (&   unused),
         imm32_t imm);
/* RexW 83  /0 RMBoth  ib   | Add sign-extended imm8 to r/m64. */
int add (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm8_t imm);
/* RexW 81  /0 RMBoth  id   | Add imm32 sign-extended to 64-bits to r/m64. */
int add (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm32_t imm);
/* RexW 81  /0 RMBoth  id   | Add imm32 sign-extended to 64-bits to r/m64. */
...
int add (code_ptr p,
         const WordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 03  /r RMBoth     | Add r/m64 to r64. */
int add (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 03  /r RMBoth     | Add r/m64 to r64. */
int add (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordPtr (&   modrm_rm));
/* RexW 01  /r RMBoth     | Add r64 to r/m64. */
int add (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const QwordReg (&   modrm_reg));
...
int add (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const BytePtr86 (&   modrm_rm));
/* Rex 02  /r RMBoth     | Add r/m8 to r8. */
int add (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
/* Rex 02  /r RMBoth     | Add r/m8 to r8. */
...
int add (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /0 RMBoth  ib   | Add sign-extended imm8 to r/m64. */
int add (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /0 RMBoth  ib   | Add sign-extended imm8 to r/m64. */
int add (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         imm8_t imm);
/* Rex 00  /r RMBoth     | Add r8 to r/m8. */
int add (code_ptr p,
         const BytePtr (&   modrm_rm),
         const ByteRegRex (&   modrm_reg));
...
int aeskeygenassist (code_ptr p,
                     ...
                     const XmmWordPtr (&   modrm_rm),
                     imm8_t imm);
/* RexW 25     id   | RAX AND imm32 sign- extended to 64-bits. */
int and_ (code_ptr p,
          const RegRAX (&   unused),
          imm32_t imm);
/* RexW 83  /4 RMBoth  ib   | r/m64 AND imm8 (sign- extended). */
int and_ (code_ptr p,
          const QwordReg_m_RAX (&   modrm_rm),
          imm8_t imm);
/* RexW 81  /4 RMBoth  id   | r/m64 AND imm32 sign extended to 64-bits. */
int and_ (code_ptr p,
          const QwordReg_m_RAX (&   modrm_rm),
          imm32_t imm);
/* RexW 81  /4 RMBoth  id   | r/m64 AND imm32 sign extended to 64-bits. */
...
int and_ (code_ptr p,
          const WordPtr (&   modrm_rm),
          imm8_t imm);
/* RexW 23  /r RMBoth     | r64 AND r/m64. */
int and_ (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordReg (&   modrm_rm));
/* RexW 23  /r RMBoth     | r64 AND r/m64. */
int and_ (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordPtr (&   modrm_rm));
/* RexW 21  /r RMBoth     | r/m64 AND r32. */
int and_ (code_ptr p,
          const QwordPtr (&   modrm_rm),
          const QwordReg (&   modrm_reg));
...
int and_ (code_ptr p,
          const ByteReg86 (&   modrm_reg),
          const BytePtr86 (&   modrm_rm));
/* Rex 22  /r RMBoth     | r/m64 AND r8 (sign- extended). */
int and_ (code_ptr p,
          const ByteRegLow (&   modrm_reg),
          const ByteRegRex (&   modrm_rm));
/* Rex 22  /r RMBoth     | r/m64 AND r8 (sign- extended). */
...
int and_ (code_ptr p,
          const BytePtr86 (&   modrm_rm),
          imm8_t imm);
/* Rex 80  /4 RMBoth  ib   | r/m64 AND imm8 (sign- extended). */
int and_ (code_ptr p,
          const ByteRegRex (&   modrm_rm),
          imm8_t imm);
/* Rex 80  /4 RMBoth  ib   | r/m64 AND imm8 (sign- extended). */
int and_ (code_ptr p,
          const BytePtrRex (&   modrm_rm),
          imm8_t imm);
/* Rex 20  /r RMBoth     | r/m64 AND r8 (sign- extended). */
int and_ (code_ptr p,
          const BytePtr (&   modrm_rm),
          const ByteRegRex (&   modrm_reg));
...
int bsf (code_ptr p,
         const DwordReg (&   modrm_reg),
         const DwordPtr (&   modrm_rm));
/* RexW 0F BC  /r RMBoth     | Bit scan forward on r/m64. */
int bsf (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 0F BC  /r RMBoth     | Bit scan forward on r/m64. */
...
int bsr (code_ptr p,
         const DwordReg (&   modrm_reg),
         const DwordPtr (&   modrm_rm));
/* RexW 0F BD  /r RMBoth     | Bit scan reverse on r/m64. */
int bsr (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 0F BD  /r RMBoth     | Bit scan reverse on r/m64. */
...
int bswap (code_ptr p,
           const DwordReg (&   radd));
/* RexW 0F C8    +rd    | Reverses the byte order of a 64-bit register. */
int bswap (code_ptr p,
           const QwordReg (&   radd));
...
int bt (code_ptr p,
        const DwordPtr (&   modrm_rm),
        const DwordReg (&   modrm_reg));
/* RexW 0F BA  /4 RMBoth  ib   | Store selected bit in CF flag. */
int bt (code_ptr p,
        const QwordReg (&   modrm_rm),
        imm8_t imm);
/* RexW 0F BA  /4 RMBoth  ib   | Store selected bit in CF flag. */
int bt (code_ptr p,
        const QwordPtr (&   modrm_rm),
        imm8_t imm);
/* RexW 0F A3  /r RMBoth     | Store selected bit in CF flag. */
int bt (code_ptr p,
        const QwordReg (&   modrm_rm),
        const QwordReg (&   modrm_reg));
/* RexW 0F A3  /r RMBoth     | Store selected bit in CF flag. */
...
int btc (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const DwordReg (&   modrm_reg));
/* RexW 0F BA  /7 RMBoth  ib   | Store selected bit in CF flag and complement. */
int btc (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW 0F BA  /7 RMBoth  ib   | Store selected bit in CF flag and complement. */
int btc (code_ptr p,
         const QwordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 0F BB  /r RMBoth     | Store selected bit in CF flag and complement. */
int btc (code_ptr p,
         const QwordReg (&   modrm_rm),
         const QwordReg (&   modrm_reg));
/* RexW 0F BB  /r RMBoth     | Store selected bit in CF flag and complement. */
...
int btr (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const DwordReg (&   modrm_reg));
/* RexW 0F BA  /6 RMBoth  ib   | Store selected bit in CF flag and clear. */
int btr (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW 0F BA  /6 RMBoth  ib   | Store selected bit in CF flag and clear. */
int btr (code_ptr p,
         const QwordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 0F B3  /r RMBoth     | Store selected bit in CF flag and clear. */
int btr (code_ptr p,
         const QwordReg (&   modrm_rm),
         const QwordReg (&   modrm_reg));
/* RexW 0F B3  /r RMBoth     | Store selected bit in CF flag and clear. */
...
int bts (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const DwordReg (&   modrm_reg));
/* RexW 0F BA  /5 RMBoth  ib   | Store selected bit in CF flag and set. */
int bts (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW 0F BA  /5 RMBoth  ib   | Store selected bit in CF flag and set. */
int bts (code_ptr p,
         const QwordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 0F AB  /r RMBoth     | Store selected bit in CF flag and set. */
int bts (code_ptr p,
         const QwordReg (&   modrm_rm),
         const QwordReg (&   modrm_reg));
/* RexW 0F AB  /r RMBoth     | Store selected bit in CF flag and set. */
...
int call (code_ptr p,
          rel32_t imm);
/*  FF  /2 RMBoth     | Call near, absolute indirect, address given in r/m64. */
int call (code_ptr p,
          const QwordReg (&   modrm_rm));
/*  FF  /2 RMBoth     | Call near, absolute indirect, address given in r/m64. */
int call (code_ptr p,
...
/* RexW FF  /3 RMMemOnly     | In 64-bit mode: If selector points to a gate, then RIP = 64-bit displacement taken from gate; else RIP = 64-bit offset from far pointer referenced in the instruction. */
int call (code_ptr p,
          const Far64Ptr (&   modrm_rm));
/*  98      op16  | AX ← sign-extend of AL. */
int cbw (code_ptr p);
/*  99      op32  | EDX:EAX ← sign-extend of EAX. */
int cdq (code_ptr p);
/* RexW 98        | RAX ← sign-extend of EAX. */
int cdqe (code_ptr p);
/*  F8        | Clear CF flag. */
int clc (code_ptr p);
/*  FC        | Clear DF flag. */
...
int cmova (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 47  /r RMBoth     | Move if above (CF=0 and ZF=0). */
int cmova (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 47  /r RMBoth     | Move if above (CF=0 and ZF=0). */
...
int cmovae (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 43  /r RMBoth     | Move if above or equal (CF=0). */
int cmovae (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 43  /r RMBoth     | Move if above or equal (CF=0). */
...
int cmovb (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 42  /r RMBoth     | Move if below (CF=1). */
int cmovb (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 42  /r RMBoth     | Move if below (CF=1). */
...
int cmovbe (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 46  /r RMBoth     | Move if below or equal (CF=1 or ZF=1). */
int cmovbe (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 46  /r RMBoth     | Move if below or equal (CF=1 or ZF=1). */
...
int cmovc (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 42  /r RMBoth     | Move if carry (CF=1). */
int cmovc (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 42  /r RMBoth     | Move if carry (CF=1). */
...
int cmove (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 44  /r RMBoth     | Move if equal (ZF=1). */
int cmove (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 44  /r RMBoth     | Move if equal (ZF=1). */
...
int cmovg (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 4F  /r RMBoth     | Move if greater (ZF=0 and SF=OF). */
int cmovg (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 4F  /r RMBoth     | Move if greater (ZF=0 and SF=OF). */
...
int cmovge (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4D  /r RMBoth     | Move if greater or equal (SF=OF). */
int cmovge (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4D  /r RMBoth     | Move if greater or equal (SF=OF). */
...
int cmovl (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 4C  /r RMBoth     | Move if less (SF≠ OF). */
int cmovl (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 4C  /r RMBoth     | Move if less (SF≠ OF). */
...
int cmovle (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4E  /r RMBoth     | Move if less or equal (ZF=1 or SF≠ OF). */
int cmovle (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4E  /r RMBoth     | Move if less or equal (ZF=1 or SF≠ OF). */
...
int cmovna (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 46  /r RMBoth     | Move if not above (CF=1 or ZF=1). */
int cmovna (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 46  /r RMBoth     | Move if not above (CF=1 or ZF=1). */
...
int cmovnae (code_ptr p,
             const DwordReg (&   modrm_reg),
             const DwordPtr (&   modrm_rm));
/* RexW 0F 42  /r RMBoth     | Move if not above or equal (CF=1). */
int cmovnae (code_ptr p,
             const QwordReg (&   modrm_reg),
             const QwordReg (&   modrm_rm));
/* RexW 0F 42  /r RMBoth     | Move if not above or equal (CF=1). */
...
int cmovnb (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 43  /r RMBoth     | Move if not below (CF=0). */
int cmovnb (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 43  /r RMBoth     | Move if not below (CF=0). */
...
int cmovnbe (code_ptr p,
             const DwordReg (&   modrm_reg),
             const DwordPtr (&   modrm_rm));
/* RexW 0F 47  /r RMBoth     | Move if not below or equal (CF=0 and ZF=0). */
int cmovnbe (code_ptr p,
             const QwordReg (&   modrm_reg),
             const QwordReg (&   modrm_rm));
/* RexW 0F 47  /r RMBoth     | Move if not below or equal (CF=0 and ZF=0). */
...
int cmovnc (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 43  /r RMBoth     | Move if not carry (CF=0). */
int cmovnc (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 43  /r RMBoth     | Move if not carry (CF=0). */
...
int cmovne (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 45  /r RMBoth     | Move if not equal (ZF=0). */
int cmovne (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 45  /r RMBoth     | Move if not equal (ZF=0). */
...
int cmovng (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4E  /r RMBoth     | Move if not greater (ZF=1 or SF≠ OF). */
int cmovng (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4E  /r RMBoth     | Move if not greater (ZF=1 or SF≠ OF). */
...
int cmovnge (code_ptr p,
             const DwordReg (&   modrm_reg),
             const DwordPtr (&   modrm_rm));
/* RexW 0F 4C  /r RMBoth     | Move if not greater or equal (SF≠ OF). */
int cmovnge (code_ptr p,
             const QwordReg (&   modrm_reg),
             const QwordReg (&   modrm_rm));
/* RexW 0F 4C  /r RMBoth     | Move if not greater or equal (SF≠ OF). */
...
int cmovnl (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4D  /r RMBoth     | Move if not less (SF=OF). */
int cmovnl (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4D  /r RMBoth     | Move if not less (SF=OF). */
...
int cmovnle (code_ptr p,
             const DwordReg (&   modrm_reg),
             const DwordPtr (&   modrm_rm));
/* RexW 0F 4F  /r RMBoth     | Move if not less or equal (ZF=0 and SF=OF). */
int cmovnle (code_ptr p,
             const QwordReg (&   modrm_reg),
             const QwordReg (&   modrm_rm));
/* RexW 0F 4F  /r RMBoth     | Move if not less or equal (ZF=0 and SF=OF). */
...
int cmovno (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 41  /r RMBoth     | Move if not overflow (OF=0). */
int cmovno (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 41  /r RMBoth     | Move if not overflow (OF=0). */
...
int cmovnp (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4B  /r RMBoth     | Move if not parity (PF=0). */
int cmovnp (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4B  /r RMBoth     | Move if not parity (PF=0). */
...
int cmovns (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 49  /r RMBoth     | Move if not sign (SF=0). */
int cmovns (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 49  /r RMBoth     | Move if not sign (SF=0). */
...
int cmovnz (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 45  /r RMBoth     | Move if not zero (ZF=0). */
int cmovnz (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 45  /r RMBoth     | Move if not zero (ZF=0). */
...
int cmovo (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 40  /r RMBoth     | Move if overflow (OF=0). */
int cmovo (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 40  /r RMBoth     | Move if overflow (OF=0). */
...
int cmovp (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 4A  /r RMBoth     | Move if parity (PF=1). */
int cmovp (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 4A  /r RMBoth     | Move if parity (PF=1). */
...
int cmovpe (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4A  /r RMBoth     | Move if parity even (PF=1). */
int cmovpe (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4A  /r RMBoth     | Move if parity even (PF=1). */
...
int cmovpo (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* RexW 0F 4B  /r RMBoth     | Move if parity odd (PF=0). */
int cmovpo (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* RexW 0F 4B  /r RMBoth     | Move if parity odd (PF=0). */
...
int cmovs (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 48  /r RMBoth     | Move if sign (SF=1). */
int cmovs (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 48  /r RMBoth     | Move if sign (SF=1). */
...
int cmovz (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 44  /r RMBoth     | Move if zero (ZF=1). */
int cmovz (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* RexW 0F 44  /r RMBoth     | Move if zero (ZF=1). */
int cmovz (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordPtr (&   modrm_rm));
/* RexW 3D     id   | Compare imm32 sign- extended to 64-bits with RAX. */
int cmp (code_ptr p,
         const RegRAX (&   unused),
         imm32_t imm);
/* RexW 83  /7 RMBoth  ib   | Compare imm8 with r/m64. */
int cmp (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm8_t imm);
/* RexW 81  /7 RMBoth  id   | Compare imm32 sign- extended to 64-bits with r/m64. */
int cmp (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm32_t imm);
/* RexW 81  /7 RMBoth  id   | Compare imm32 sign- extended to 64-bits with r/m64. */
...
int cmp (code_ptr p,
         const WordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 3B  /r RMBoth     | Compare r/m64 with r64. */
int cmp (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 3B  /r RMBoth     | Compare r/m64 with r64. */
int cmp (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordPtr (&   modrm_rm));
/* RexW 39  /r RMBoth     | Compare r64 with r/m64. */
int cmp (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const QwordReg (&   modrm_reg));
...
int cmp (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const BytePtr86 (&   modrm_rm));
/* Rex 3A  /r RMBoth     | Compare r/m8 with r8. */
int cmp (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
/* Rex 3A  /r RMBoth     | Compare r/m8 with r8. */
...
int cmp (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /7 RMBoth  ib   | Compare imm8 with r/m8. */
int cmp (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /7 RMBoth  ib   | Compare imm8 with r/m8. */
int cmp (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         imm8_t imm);
/* Rex 38  /r RMBoth     | Compare r8 with r/m8. */
int cmp (code_ptr p,
         const BytePtr (&   modrm_rm),
         const ByteRegRex (&   modrm_reg));
...
int cmps (code_ptr p,
          const BytePtr_RSI (&   ptr),
          const BytePtr_ES_RDI (&   unused));
/*  A6        | For legacy mode, compare byte at address DS:(E)SI with byte at address ES:(E)DI; For 64-bit mode compare byte at address (R|E)SI to byte at address (R|E)DI. The status flags are set accordingly. */
int cmps (code_ptr p,
...
int cmps (code_ptr p,
          const WordPtr_RSI (&   ptr),
          const WordPtr_ES_RDI (&   unused));
/*  A7      op16  | For legacy mode, compare word at address DS:(E)SI with word at address ES:(E)DI; For 64-bit mode compare word at address (R|E)SI with word at address (R|E)DI. The status flags are set accordingly. */
int cmps (code_ptr p,
...
int cmps (code_ptr p,
          const DwordPtr_RSI (&   ptr),
          const DwordPtr_ES_RDI (&   unused));
/*  A7      op32  | For legacy mode, compare dword at address DS:(E)SI at dword at address ES:(E)DI; For 64-bit mode compare dword at address (R|E)SI at dword at address (R|E)DI. The status flags are set accordingly. */
int cmps (code_ptr p,
          const DwordPtr_ESI (&   ptr),
          const DwordPtr_ES_EDI (&   unused));
/* RexW A7        | Compares quadword at address (R|E)SI with quadword at address (R|E)DI and sets the status flags accordingly. */
int cmps (code_ptr p,
          const QwordPtr_RSI (&   ptr),
          const QwordPtr_ES_RDI (&   unused));
/* RexW A7        | Compares quadword at address (R|E)SI with quadword at address (R|E)DI and sets the status flags accordingly. */
int cmps (code_ptr p,
          const QwordPtr_ESI (&   ptr),
          const QwordPtr_ES_EDI (&   unused));
...
int cmpxchg (code_ptr p,
             const ByteReg86 (&   modrm_rm),
             const ByteReg86 (&   modrm_reg));
/* Rex 0F B0  /r RMBoth     | Compare AL with r/m8. If equal, ZF is set and r8 is loaded into r/m8. Else, clear ZF and load r/m8 into AL. */
int cmpxchg (code_ptr p,
             const ByteRegLow (&   modrm_rm),
             const ByteRegRex (&   modrm_reg));
/* Rex 0F B0  /r RMBoth     | Compare AL with r/m8. If equal, ZF is set and r8 is loaded into r/m8. Else, clear ZF and load r/m8 into AL. */
...
int cmpxchg (code_ptr p,
             const DwordPtr (&   modrm_rm),
             const DwordReg (&   modrm_reg));
/* RexW 0F B1  /r RMBoth     | Compare RAX with r/m64. If equal, ZF is set and r64 is loaded into r/m64. Else, clear ZF and load r/m64 into RAX. */
int cmpxchg (code_ptr p,
             const QwordReg (&   modrm_rm),
             const QwordReg (&   modrm_reg));
/* RexW 0F B1  /r RMBoth     | Compare RAX with r/m64. If equal, ZF is set and r64 is loaded into r/m64. Else, clear ZF and load r/m64 into RAX. */
...
int cmpxchg (code_ptr p,
             const BytePtrRex (&   modrm_rm),
             const ByteRegLow (&   modrm_reg));
/* RexW 0F C7  /1 RMMemOnly     | Compare RDX:RAX with m128. If equal, set ZF and load RCX:RBX into m128. Else, clear ZF and load m128 into RDX:RAX. */
int cmpxchg16b (code_ptr p,
...
/* RexW 99        | RDX:RAX← sign-extend of RAX. */
int cqo (code_ptr p);
/* F2 Rex 0F 38 F0  /r RMBoth     | Accumulate CRC32 on r/m8. */
...
int crc32 (code_ptr p,
           const DwordRegRex (&   modrm_reg),
           const BytePtrRex (&   modrm_rm));
/* F2 RexW 0F 38 F0  /r RMBoth     | Accumulate CRC32 on r/m8. */
int crc32 (code_ptr p,
           const QwordReg (&   modrm_reg),
           const ByteRegLow (&   modrm_rm));
/* F2 RexW 0F 38 F0  /r RMBoth     | Accumulate CRC32 on r/m8. */
int crc32 (code_ptr p,
           const QwordReg (&   modrm_reg),
           const ByteRegRex (&   modrm_rm));
/* F2 RexW 0F 38 F1  /r RMBoth     | Accumulate CRC32 on r/m64. */
int crc32 (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordReg (&   modrm_rm));
/* F2 RexW 0F 38 F1  /r RMBoth     | Accumulate CRC32 on r/m64. */
...
int cvtsd2si (code_ptr p,
              const DwordReg (&   modrm_reg),
              const QwordPtr (&   modrm_rm));
/* F2 RexW 0F 2D  /r RMBoth     | Convert one double- precision floating-point value from xmm/m64 to one signed quadword integer sign-extended into r64. */
int cvtsd2si (code_ptr p,
              const QwordReg (&   modrm_reg),
              const XmmReg (&   modrm_rm));
/* F2 RexW 0F 2D  /r RMBoth     | Convert one double- precision floating-point value from xmm/m64 to one signed quadword integer sign-extended into r64. */
...
int cvtsi2sd (code_ptr p,
              const XmmReg (&   modrm_reg),
              const DwordPtr (&   modrm_rm));
/* F2 RexW 0F 2A  /r RMBoth     | Convert one signed quadword integer from r/m64 to one double- precision floating-point value in xmm. */
int cvtsi2sd (code_ptr p,
              const XmmReg (&   modrm_reg),
              const QwordReg (&   modrm_rm));
/* F2 RexW 0F 2A  /r RMBoth     | Convert one signed quadword integer from r/m64 to one double- precision floating-point value in xmm. */
...
int cvtsi2ss (code_ptr p,
              const XmmReg (&   modrm_reg),
              const DwordPtr (&   modrm_rm));
/* F3 RexW 0F 2A  /r RMBoth     | Convert one signed quadword integer from r/m64 to one single- precision floating-point value in xmm. */
int cvtsi2ss (code_ptr p,
              const XmmReg (&   modrm_reg),
              const QwordReg (&   modrm_rm));
/* F3 RexW 0F 2A  /r RMBoth     | Convert one signed quadword integer from r/m64 to one single- precision floating-point value in xmm. */
...
int cvtss2si (code_ptr p,
              const DwordReg (&   modrm_reg),
              const DwordPtr (&   modrm_rm));
/* F3 RexW 0F 2D  /r RMBoth     | Convert one single-precision floating-point value from xmm/m32 to one signed quadword integer in r64. */
int cvtss2si (code_ptr p,
              const QwordReg (&   modrm_reg),
              const XmmReg (&   modrm_rm));
/* F3 RexW 0F 2D  /r RMBoth     | Convert one single-precision floating-point value from xmm/m32 to one signed quadword integer in r64. */
...
int cvttsd2si (code_ptr p,
               const DwordReg (&   modrm_reg),
               const QwordPtr (&   modrm_rm));
/* F2 RexW 0F 2C  /r RMBoth     | Convert one double precision floating-point value from xmm/m64 to one signedquadword integer in r64 using truncation. */
int cvttsd2si (code_ptr p,
               const QwordReg (&   modrm_reg),
               const XmmReg (&   modrm_rm));
/* F2 RexW 0F 2C  /r RMBoth     | Convert one double precision floating-point value from xmm/m64 to one signedquadword integer in r64 using truncation. */
...
int cvttss2si (code_ptr p,
               const DwordReg (&   modrm_reg),
               const DwordPtr (&   modrm_rm));
/* F3 RexW 0F 2C  /r RMBoth     | Convert one single-precision floating-point value from xmm/m32 to one signed quadword integer in r64 using truncation. */
int cvttss2si (code_ptr p,
               const QwordReg (&   modrm_reg),
               const XmmReg (&   modrm_rm));
/* F3 RexW 0F 2C  /r RMBoth     | Convert one single-precision floating-point value from xmm/m32 to one signed quadword integer in r64 using truncation. */
...
int dec (code_ptr p,
         const BytePtr86 (&   modrm_rm));
/* Rex FE  /1 RMBoth     | Decrement r/m8 by 1. */
int dec (code_ptr p,
         const ByteRegRex (&   modrm_rm));
/* Rex FE  /1 RMBoth     | Decrement r/m8 by 1. */
int dec (code_ptr p,
...
int dec (code_ptr p,
         const DwordPtr (&   modrm_rm));
/* RexW FF  /1 RMBoth     | Decrement r/m64 by 1. */
int dec (code_ptr p,
         const QwordReg (&   modrm_rm));
/* RexW FF  /1 RMBoth     | Decrement r/m64 by 1. */
int dec (code_ptr p,
...
int div (code_ptr p,
         const BytePtr86 (&   modrm_rm));
/* Rex F6  /6 RMBoth     | Unsigned divide AX by r/m8, with result stored in AL ← Quotient, AH ← Remainder. */
int div (code_ptr p,
         const ByteRegRex (&   modrm_rm));
/* Rex F6  /6 RMBoth     | Unsigned divide AX by r/m8, with result stored in AL ← Quotient, AH ← Remainder. */
int div (code_ptr p,
...
int div (code_ptr p,
         const DwordPtr (&   modrm_rm));
/* RexW F7  /6 RMBoth     | Unsigned divide RDX:RAX by r/m64, with result stored in RAX ← Quotient, RDX ← Remainder. */
int div (code_ptr p,
         const QwordReg (&   modrm_rm));
/* RexW F7  /6 RMBoth     | Unsigned divide RDX:RAX by r/m64, with result stored in RAX ← Quotient, RDX ← Remainder. */
int div (code_ptr p,
...
int fxrstor (code_ptr p,
             const VoidPtr (&   modrm_rm));
/* RexW 0F AE  /1 RMMemOnly     | Restore the x87 FPU, MMX, XMM, and MXCSR register state from m512byte. */
int fxrstor64 (code_ptr p,
               const VoidPtr (&   modrm_rm));
/*  0F AE  /0 RMMemOnly     | Save the x87 FPU, MMX, XMM, and MXCSR register state to m512byte. */
int fxsave (code_ptr p,
            const VoidPtr (&   modrm_rm));
/* RexW 0F AE  /0 RMMemOnly     | Save the x87 FPU, MMX, XMM, and MXCSR register state to m512byte. */
int fxsave64 (code_ptr p,
              const VoidPtr (&   modrm_rm));
...
int idiv (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex F6  /7 RMBoth     | Signed divide AX by r/m8, with result stored in AL ← Quotient, AH ← Remainder. */
int idiv (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex F6  /7 RMBoth     | Signed divide AX by r/m8, with result stored in AL ← Quotient, AH ← Remainder. */
int idiv (code_ptr p,
...
int idiv (code_ptr p,
          const DwordPtr (&   modrm_rm));
/* RexW F7  /7 RMBoth     | Signed divide RDX:RAX by r/m64, with result stored in RAX ← Quotient, RDX ← Remainder. */
int idiv (code_ptr p,
          const QwordReg (&   modrm_rm));
/* RexW F7  /7 RMBoth     | Signed divide RDX:RAX by r/m64, with result stored in RAX ← Quotient, RDX ← Remainder. */
int idiv (code_ptr p,
...
int imul (code_ptr p,
          const DwordPtr (&   modrm_rm));
/* RexW F7  /5 RMBoth     | RDX:RAX ← RAX ∗ r/m64. */
int imul (code_ptr p,
          const QwordReg (&   modrm_rm));
/* RexW F7  /5 RMBoth     | RDX:RAX ← RAX ∗ r/m64. */
int imul (code_ptr p,
...
int imul (code_ptr p,
          const DwordReg (&   modrm_reg),
          const DwordPtr (&   modrm_rm));
/* RexW 0F AF  /r RMBoth     | Quadword register ← Quadword register ∗ r/m64. */
int imul (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordReg (&   modrm_rm));
/* RexW 0F AF  /r RMBoth     | Quadword register ← Quadword register ∗ r/m64. */
...
int imul (code_ptr p,
          ...
          const DwordPtr (&   modrm_rm),
          imm32_t imm);
/* RexW 6B  /r RMBoth  ib   | Quadword register ← r/m64 ∗ sign-extended immediate byte. */
int imul (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordReg (&   modrm_rm),
          imm8_t imm);
...
int imul (code_ptr p,
          ...
          const QwordPtr (&   modrm_rm),
          imm8_t imm);
/* RexW 69  /r RMBoth  id   | Quadword register ← r/m64 ∗ immediate doubleword. */
int imul (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordReg (&   modrm_rm),
          imm32_t imm);
...
int inc (code_ptr p,
         const BytePtr86 (&   modrm_rm));
/* Rex FE  /0 RMBoth     | Increment r/m byte by 1. */
int inc (code_ptr p,
         const ByteRegRex (&   modrm_rm));
/* Rex FE  /0 RMBoth     | Increment r/m byte by 1. */
int inc (code_ptr p,
...
int inc (code_ptr p,
         const DwordPtr (&   modrm_rm));
/* RexW FF  /0 RMBoth     | Increment r/m quadword by 1. */
int inc (code_ptr p,
         const QwordReg (&   modrm_rm));
/* RexW FF  /0 RMBoth     | Increment r/m quadword by 1. */
int inc (code_ptr p,
...
int invd (code_ptr p);
/* 66 0F 38 80  /r RMMemOnly     | Invalidates EPT-derived entries in the TLBs and paging-structure caches (in 64-bit mode) */
...
int invlpg (code_ptr p,
            const VoidPtr (&   modrm_rm));
/* 66 0F 38 81  /r RMMemOnly     | Invalidates entries in the TLBs and paging-structure caches based on VPID (in 64-bit mode) */
...
int lar (code_ptr p,
         const DwordReg (&   modrm_reg),
         const WordPtr (&   modrm_rm));
/* RexW 0F 02  /r RMBoth     | r64 ← r32/m16 masked by 00FxFF00H and zero extended */
int lar (code_ptr p,
         const QwordReg (&   modrm_reg),
         const DwordReg (&   modrm_rm));
/* RexW 0F 02  /r RMBoth     | r64 ← r32/m16 masked by 00FxFF00H and zero extended */
...
int lea (code_ptr p,
         const DwordReg (&   modrm_reg),
         const VoidPtr (&   modrm_rm));
/* RexW 8D  /r RMMemOnly     | Store effective address for m in register r64. */
int lea (code_ptr p,
         const QwordReg (&   modrm_reg),
         const VoidPtr (&   modrm_rm));
/*  C9      op32  | Set RSP to RBP, then pop RBP. */
...
int lfs (code_ptr p,
         const DwordReg (&   modrm_reg),
         const Far32Ptr (&   modrm_rm));
/* RexW 0F B4  /r RMMemOnly     | Load FS:r64 with far pointer from memory. */
int lfs (code_ptr p,
         const QwordReg (&   modrm_reg),
         const Far64Ptr (&   modrm_rm));
...
int lgs (code_ptr p,
         const DwordReg (&   modrm_reg),
         const Far32Ptr (&   modrm_rm));
/* RexW 0F B5  /r RMMemOnly     | Load GS:r64 with far pointer from memory. */
int lgs (code_ptr p,
         const QwordReg (&   modrm_reg),
         const Far64Ptr (&   modrm_rm));
...
int lods (code_ptr p,
          const RegEAX (&   unused),
          const DwordPtr_ESI (&   ptr));
/* RexW AD        | Load qword at address (R)SI into RAX. */
int lods (code_ptr p,
          const QwordPtr_RSI (&   ptr));
/* RexW AD        | Load qword at address (R)SI into RAX. */
int lods (code_ptr p,
...
int lsl (code_ptr p,
         const DwordReg (&   modrm_reg),
         const WordPtr (&   modrm_rm));
/* RexW 0F 03  /r RMBoth     | Load: r64 ← segment limit, selector r32/m16 */
int lsl (code_ptr p,
         const QwordReg (&   modrm_reg),
         const DwordReg (&   modrm_rm));
/* RexW 0F 03  /r RMBoth     | Load: r64 ← segment limit, selector r32/m16 */
...
int lss (code_ptr p,
         const DwordReg (&   modrm_reg),
         const Far32Ptr (&   modrm_rm));
/* RexW 0F B2  /r RMMemOnly     | Load SS:r64 with far pointer from memory. */
int lss (code_ptr p,
         const QwordReg (&   modrm_reg),
         const Far64Ptr (&   modrm_rm));
...
/* RexW 8B  /r RMBoth     | Move r/m64 to r64. */
int mov (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 8B  /r RMBoth     | Move r/m64 to r64. */
int mov (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordPtr (&   modrm_rm));
/* RexW 89  /r RMBoth     | Move r64 to r/m64. */
int mov (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const QwordReg (&   modrm_reg));
/* RexW B8    +rd io   | Move imm64 to r64. */
int mov (code_ptr p,
         const QwordReg (&   radd),
         imm64_t imm);
/* RexW C7  /0 RMBoth  id   | Move imm32 sign extended to 64-bits to r/m64. */
int mov (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm32_t imm);
/* RexW C7  /0 RMBoth  id   | Move imm32 sign extended to 64-bits to r/m64. */
...
int mov (code_ptr p,
         const ByteReg86 (&   radd),
         imm8_t imm);
/* Rex B0    +rb ib   | Move imm8 to r8. */
int mov (code_ptr p,
         const ByteRegRex (&   radd),
         imm8_t imm);
/*  C6  /0 RMBoth  ib   | Move imm8 to r/m8. */
int mov (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C6  /0 RMBoth  ib   | Move imm8 to r/m8. */
int mov (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         imm8_t imm);
...
int mov (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const BytePtr86 (&   modrm_rm));
/* Rex 8A  /r RMBoth     | Move r/m8 to r8. */
int mov (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
/* Rex 8A  /r RMBoth     | Move r/m8 to r8. */
...
int mov (code_ptr p,
         const ByteRegRex (&   modrm_reg),
         const BytePtr (&   modrm_rm));
/*  0F 20  /r RMRegOnly     | Move extended control register to r64. */
int mov (code_ptr p,
         const QwordReg (&   modrm_rm),
         const CrReg (&   modrm_reg));
/*  0F 21  /r RMRegOnly     | Move extended debug register to r64. */
int mov (code_ptr p,
         const QwordReg (&   modrm_rm),
         const DrReg (&   modrm_reg));
/*  0F 22  /r RMRegOnly     | Move r64 to extended control register. */
int mov (code_ptr p,
         const CrReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/*  0F 23  /r RMRegOnly     | Move r64 to extended debug register. */
int mov (code_ptr p,
         const DrReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
...
int mov (code_ptr p,
         const SegReg (&   modrm_reg),
         const WordPtr (&   modrm_rm));
/* RexW 8E  /r RMBoth     | Move lower 16 bits of r/m64 to segment register. */
int mov (code_ptr p,
         const SegReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 8E  /r RMBoth     | Move lower 16 bits of r/m64 to segment register. */
...
int mov (code_ptr p,
         const RegEAX (&   unused),
         const DwordOffset64 (&   offset));
/*  A1      op32  | Move doubleword at (seg:offset) to EAX. */
int mov (code_ptr p,
         const RegEAX (&   unused),
         const DwordOffset32 (&   offset));
/* RexW A1        | Move quadword at (offset) to RAX. */
int mov (code_ptr p,
         const RegRAX (&   unused),
         const QwordOffset64 (&   offset));
/* RexW A1        | Move quadword at (offset) to RAX. */
int mov (code_ptr p,
         const RegRAX (&   unused),
         const QwordOffset32 (&   offset));
...
int mov (code_ptr p,
         const WordPtr (&   modrm_rm),
         const SegReg (&   modrm_reg));
/* RexW 8C  /r RMBoth     | Move zero extended 16-bit segment register to r/m64. */
int mov (code_ptr p,
         const QwordReg (&   modrm_rm),
         const SegReg (&   modrm_reg));
/* RexW 8C  /r RMBoth     | Move zero extended 16-bit segment register to r/m64. */
int mov (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const SegReg (&   modrm_reg));
/* Rex 88  /r RMBoth     | Move r8 to r/m8. */
int mov (code_ptr p,
         const BytePtr (&   modrm_rm),
         const ByteRegRex (&   modrm_reg));
...
int movbe (code_ptr p,
           const DwordReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW 0F 38 F0  /r RMMemOnly     | Reverse byte order in m64 and move to r64. */
int movbe (code_ptr p,
           const QwordReg (&   modrm_reg),
           const QwordPtr (&   modrm_rm));
...
int movbe (code_ptr p,
           const DwordPtr (&   modrm_rm),
           const DwordReg (&   modrm_reg));
/* RexW 0F 38 F1  /r RMMemOnly     | Reverse byte order in r64 and move to m64. */
int movbe (code_ptr p,
           const QwordPtr (&   modrm_rm),
           const QwordReg (&   modrm_reg));
...
int movnti (code_ptr p,
            const DwordPtr (&   modrm_rm),
            const DwordReg (&   modrm_reg));
/* RexW 0F C3  /r RMMemOnly     | Move quadword from r64 to m64 using non-temporal hint. */
int movnti (code_ptr p,
            const QwordPtr (&   modrm_rm),
            const QwordReg (&   modrm_reg));
...
int movq (code_ptr p,
          const QwordPtr (&   modrm_rm),
          const XmmReg (&   modrm_reg));
/* 66 RexW 0F 7E  /r RMBoth     | Move quadword from xmm register to r/m64. */
int movq (code_ptr p,
          const QwordReg (&   modrm_rm),
          const XmmReg (&   modrm_reg));
...
int movq (code_ptr p,
          const XmmReg (&   modrm_reg),
          const XmmReg (&   modrm_rm));
/* 66 RexW 0F 6E  /r RMBoth     | Move quadword from r/m64 to xmm. */
int movq (code_ptr p,
          const XmmReg (&   modrm_reg),
          const QwordReg (&   modrm_rm));
/* F3 0F 7E  /r RMBoth     | Move quadword from xmm2/mem64 to xmm1. */
...
int movq (code_ptr p,
          const QwordPtr (&   modrm_rm),
          const MmReg (&   modrm_reg));
/* RexW 0F 7E  /r RMBoth     | Move quadword from mm to r/m64. */
int movq (code_ptr p,
          const QwordReg (&   modrm_rm),
          const MmReg (&   modrm_reg));
...
int movq (code_ptr p,
          const MmReg (&   modrm_reg),
          const MmReg (&   modrm_rm));
/* RexW 0F 6E  /r RMBoth     | Move quadword from r/m64 to mm. */
int movq (code_ptr p,
          const MmReg (&   modrm_reg),
          const QwordReg (&   modrm_rm));
/*  0F 6F  /r RMBoth     | Move quadword from mm/m64 to mm. */
...
int movs (code_ptr p,
          const DwordPtr_ES_EDI (&   unused),
          const DwordPtr_ESI (&   ptr));
/* RexW A5        | Move qword from address (R|E)SI to (R|E)DI. */
int movs (code_ptr p,
          const QwordPtr_ES_RDI (&   unused),
          const QwordPtr_RSI (&   ptr));
/* RexW A5        | Move qword from address (R|E)SI to (R|E)DI. */
...
int movsx (code_ptr p,
           const WordRegRex (&   modrm_reg),
           const BytePtrRex (&   modrm_rm));
/* Rex 0F BE  /r RMBoth   op32  | see MOVSX r32,r/m8 */
int movsx (code_ptr p,
           const DwordReg (&   modrm_reg),
           const ByteRegRex (&   modrm_rm));
...
int movsx (code_ptr p,
           const DwordRegRex (&   modrm_reg),
           const BytePtrRex (&   modrm_rm));
/* RexW 0F BE  /r RMBoth     | Move byte to quadword with sign-extension. */
int movsx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const ByteRegLow (&   modrm_rm));
/* RexW 0F BE  /r RMBoth     | Move byte to quadword with sign-extension. */
int movsx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const ByteRegRex (&   modrm_rm));
/* RexW 0F BF  /r RMBoth     | Move word to quadword with sign-extension. */
int movsx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const WordReg (&   modrm_rm));
/* RexW 0F BF  /r RMBoth     | Move word to quadword with sign-extension. */
...
int movsx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const BytePtr (&   modrm_rm));
/* RexW 63  /r RMBoth     | Move doubleword to quadword with sign- extension. */
int movsxd (code_ptr p,
            const QwordReg (&   modrm_reg),
            const DwordReg (&   modrm_rm));
/* RexW 63  /r RMBoth     | Move doubleword to quadword with sign- extension. */
int movsxd (code_ptr p,
...
int movups (code_ptr p,
            const XmmWordPtr (&   modrm_rm),
            const XmmReg (&   modrm_reg));
/* Rex 0F B6  /r RMBoth   op16  | see MOVZX r16,r/m8 */
...
int movzx (code_ptr p,
           const WordRegRex (&   modrm_reg),
           const BytePtrRex (&   modrm_rm));
/* Rex 0F B6  /r RMBoth   op32  | see MOVZX r32,r/m8 */
int movzx (code_ptr p,
           const DwordReg (&   modrm_reg),
           const ByteRegRex (&   modrm_rm));
...
int movzx (code_ptr p,
           const DwordRegRex (&   modrm_reg),
           const BytePtrRex (&   modrm_rm));
/* RexW 0F B6  /r RMBoth     | Move byte to quadword, zero-extension. */
int movzx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const ByteRegLow (&   modrm_rm));
/* RexW 0F B6  /r RMBoth     | Move byte to quadword, zero-extension. */
int movzx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const ByteRegRex (&   modrm_rm));
/* RexW 0F B7  /r RMBoth     | Move word to quadword, zero-extension. */
int movzx (code_ptr p,
           const QwordReg (&   modrm_reg),
           const WordReg (&   modrm_rm));
/* RexW 0F B7  /r RMBoth     | Move word to quadword, zero-extension. */
...
int mul (code_ptr p,
         const BytePtr86 (&   modrm_rm));
/* Rex F6  /4 RMBoth     | Unsigned multiply (AX ← AL ∗ r/m8). */
int mul (code_ptr p,
         const ByteRegRex (&   modrm_rm));
/* Rex F6  /4 RMBoth     | Unsigned multiply (AX ← AL ∗ r/m8). */
int mul (code_ptr p,
...
int mul (code_ptr p,
         const DwordPtr (&   modrm_rm));
/* RexW F7  /4 RMBoth     | Unsigned multiply (RDX:RAX ← RAX ∗ r/m64. */
int mul (code_ptr p,
         const QwordReg (&   modrm_rm));
/* RexW F7  /4 RMBoth     | Unsigned multiply (RDX:RAX ← RAX ∗ r/m64. */
int mul (code_ptr p,
...
int neg (code_ptr p,
         const BytePtr86 (&   modrm_rm));
/* Rex F6  /3 RMBoth     | Two's complement negate r/m8. */
int neg (code_ptr p,
         const ByteRegRex (&   modrm_rm));
/* Rex F6  /3 RMBoth     | Two's complement negate r/m8. */
int neg (code_ptr p,
...
int neg (code_ptr p,
         const DwordPtr (&   modrm_rm));
/* RexW F7  /3 RMBoth     | Two's complement negate r/m64. */
int neg (code_ptr p,
         const QwordReg (&   modrm_rm));
/* RexW F7  /3 RMBoth     | Two's complement negate r/m64. */
int neg (code_ptr p,
...
int not_ (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex F6  /2 RMBoth     | Reverse each bit of r/m8. */
int not_ (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex F6  /2 RMBoth     | Reverse each bit of r/m8. */
int not_ (code_ptr p,
...
int not_ (code_ptr p,
          const DwordPtr (&   modrm_rm));
/* RexW F7  /2 RMBoth     | Reverse each bit of r/m64. */
int not_ (code_ptr p,
          const QwordReg (&   modrm_rm));
/* RexW F7  /2 RMBoth     | Reverse each bit of r/m64. */
int not_ (code_ptr p,
          const QwordPtr (&   modrm_rm));
/* RexW 0D     id   | RAX OR imm32 (sign- extended). */
int or_ (code_ptr p,
         const RegRAX (&   unused),
         imm32_t imm);
/* RexW 83  /1 RMBoth  ib   | r/m64 OR imm8 (sign- extended). */
int or_ (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm8_t imm);
/* RexW 81  /1 RMBoth  id   | r/m64 OR imm32 (sign- extended). */
int or_ (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm32_t imm);
/* RexW 81  /1 RMBoth  id   | r/m64 OR imm32 (sign- extended). */
...
int or_ (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /1 RMBoth  ib   | r/m8 OR imm8. */
int or_ (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /1 RMBoth  ib   | r/m8 OR imm8. */
...
int pextrd (code_ptr p,
            ...
            const XmmReg (&   modrm_reg),
            imm8_t imm);
/* 66 RexW 0F 3A 16  /r RMBoth  ib   | Extract a qword integer value from xmm2 at the source qword offset specified by imm8 into r/m64. */
int pextrq (code_ptr p,
            const QwordReg (&   modrm_rm),
            const XmmReg (&   modrm_reg),
            imm8_t imm);
/* 66 RexW 0F 3A 16  /r RMBoth  ib   | Extract a qword integer value from xmm2 at the source qword offset specified by imm8 into r/m64. */
...
int pmovmskb (code_ptr p,
              const DwordReg (&   modrm_reg),
              const MmReg (&   modrm_rm));
/* RexW 0F D7  /r RMRegOnly     | Move a byte mask of mm to the lower 32-bits of r64 and zero-fill the upper 32-bits. */
int pmovmskb (code_ptr p,
              const QwordReg (&   modrm_reg),
              const MmReg (&   modrm_rm));
...
int pmuludq (code_ptr p,
             const XmmReg (&   modrm_reg),
             const XmmWordPtr (&   modrm_rm));
/*  58    +rd    | Pop top of stack into r64; increment stack pointer. Cannot encode 32-bit operand size. */
int pop (code_ptr p,
         const QwordReg (&   radd));
/*  8F  /0 RMBoth     | Pop top of stack into m64; increment stack pointer. Cannot encode 32-bit operand size. */
int pop (code_ptr p,
         const QwordPtr (&   modrm_rm));
...
int popcnt (code_ptr p,
            const DwordReg (&   modrm_reg),
            const DwordPtr (&   modrm_rm));
/* F3 RexW 0F B8  /r RMBoth     | POPCNT on r/m64 */
int popcnt (code_ptr p,
            const QwordReg (&   modrm_reg),
            const QwordReg (&   modrm_rm));
/* F3 RexW 0F B8  /r RMBoth     | POPCNT on r/m64 */
...
int popf (code_ptr p);
/* RexW 9D        | Pop top of stack and zero- extend into RFLAGS. */
int popfq (code_ptr p);
...
int punpcklwd (code_ptr p,
               const XmmReg (&   modrm_reg),
               const XmmWordPtr (&   modrm_rm));
/*  50    +rd    | Push r64. Default operand size 64-bits. */
int push (code_ptr p,
          const QwordReg (&   radd));
/*  FF  /6 RMBoth     | Push r/m64. Default operand size 64-bits. */
int push (code_ptr p,
          const QwordPtr (&   modrm_rm));
/*  50    +rw  op16  | Push r16. */
int push (code_ptr p,
...
int pxor (code_ptr p,
          const XmmReg (&   modrm_reg),
          const XmmWordPtr (&   modrm_rm));
/* RexW C1  /2 RMBoth  ib   | Rotate 65 bits (CF, r/m64) left imm8 times. Uses a 6 bit count. */
int rcl (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /2 RMBoth  ib   | Rotate 65 bits (CF, r/m64) left imm8 times. Uses a 6 bit count. */
...
int rcl (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /2 RMBoth  ib   | Rotate 9 bits (CF, r/m8) left imm8 times. */
int rcl (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /2 RMBoth  ib   | Rotate 9 bits (CF, r/m8) left imm8 times. */
...
int rcl (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /2 RMBoth     | Rotate 9 bits (CF, r/m8) left CL times. */
int rcl (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /2 RMBoth     | Rotate 9 bits (CF, r/m8) left CL times. */
...
int rcl (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /2 RMBoth     | Rotate 65 bits (CF, r/m64) left CL times. Uses a 6 bit count. */
int rcl (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /2 RMBoth     | Rotate 65 bits (CF, r/m64) left CL times. Uses a 6 bit count. */
...
int rcpss (code_ptr p,
           const XmmReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/* RexW C1  /3 RMBoth  ib   | Rotate 65 bits (CF, r/m64) right imm8 times. Uses a 6 bit count. */
int rcr (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /3 RMBoth  ib   | Rotate 65 bits (CF, r/m64) right imm8 times. Uses a 6 bit count. */
...
int rcr (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /3 RMBoth  ib   | Rotate 9 bits (CF, r/m8) right imm8 times. */
int rcr (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /3 RMBoth  ib   | Rotate 9 bits (CF, r/m8) right imm8 times. */
...
int rcr (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /3 RMBoth     | Rotate 9 bits (CF, r/m8) right CL times. */
int rcr (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /3 RMBoth     | Rotate 9 bits (CF, r/m8) right CL times. */
...
int rcr (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /3 RMBoth     | Rotate 65 bits (CF, r/m64) right CL times. Uses a 6 bit count. */
int rcr (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /3 RMBoth     | Rotate 65 bits (CF, r/m64) right CL times. Uses a 6 bit count. */
...
int repe_cmps (code_ptr p,
               const BytePtr_ESI (&   ptr),
               const BytePtr_ES_EDI (&   unused));
/* F3 RexW A6        | Find non-matching bytes in [RDI] and [RSI]. */
int rexw_repe_cmps (code_ptr p,
                    const BytePtr_RSI (&   ptr),
                    const BytePtr_ES_RDI (&   unused));
/* F3 RexW A6        | Find non-matching bytes in [RDI] and [RSI]. */
int rexw_repe_cmps (code_ptr p,
...
int repe_cmps (code_ptr p,
               const DwordPtr_ESI (&   ptr),
               const DwordPtr_ES_EDI (&   unused));
/* F3 RexW A7        | Find non-matching quadwords in [RDI] and [RSI]. */
int repe_cmps (code_ptr p,
               const QwordPtr_RSI (&   ptr),
               const QwordPtr_ES_RDI (&   unused));
/* F3 RexW A7        | Find non-matching quadwords in [RDI] and [RSI]. */
...
/* F3 RexW AE        | Find non-AL byte starting at [RDI]. */
int rexw_repe_scas (code_ptr p,
                    const BytePtr_ES_RDI (&   unused));
/* F3 RexW AE        | Find non-AL byte starting at [RDI]. */
int rexw_repe_scas (code_ptr p,
                    const RegAL (&   unused1),
...
/* F3 RexW AF        | Find non-RAX quadword starting at [RDI]. */
int repe_scas (code_ptr p,
               const QwordPtr_ES_RDI (&   unused));
/* F3 RexW AF        | Find non-RAX quadword starting at [RDI]. */
int repe_scas (code_ptr p,
               const RegRAX (&   unused1),
               const QwordPtr_ES_RDI (&   unused2));
/* F3 RexW AF        | Find non-RAX quadword starting at [RDI]. */
int repe_scas (code_ptr p,
               const QwordPtr_ES_EDI (&   unused));
/* F3 RexW AF        | Find non-RAX quadword starting at [RDI]. */
...
int repne_cmps (code_ptr p,
                const BytePtr_ESI (&   ptr),
                const BytePtr_ES_EDI (&   unused));
/* F2 RexW A6        | Find matching bytes in [RDI] and [RSI]. */
int rexw_repne_cmps (code_ptr p,
                     const BytePtr_RSI (&   ptr),
                     const BytePtr_ES_RDI (&   unused));
/* F2 RexW A6        | Find matching bytes in [RDI] and [RSI]. */
int rexw_repne_cmps (code_ptr p,
...
int repne_cmps (code_ptr p,
                const DwordPtr_ESI (&   ptr),
                const DwordPtr_ES_EDI (&   unused));
/* F2 RexW A7        | Find matching doublewords in [RDI] and [RSI]. */
int repne_cmps (code_ptr p,
                const QwordPtr_RSI (&   ptr),
                const QwordPtr_ES_RDI (&   unused));
/* F2 RexW A7        | Find matching doublewords in [RDI] and [RSI]. */
...
int repne_scas (code_ptr p,
                const RegAL (&   unused1),
                const BytePtr_ES_EDI (&   unused2));
/* F2 RexW AE        | Find AL, starting at [RDI]. */
int rexw_repne_scas (code_ptr p,
                     const BytePtr_ES_RDI (&   unused));
/* F2 RexW AE        | Find AL, starting at [RDI]. */
int rexw_repne_scas (code_ptr p,
                     const RegAL (&   unused1),
...
int repne_scas (code_ptr p,
                const RegEAX (&   unused1),
                const DwordPtr_ES_EDI (&   unused2));
/* F2 RexW AF        | Find RAX, starting at [RDI]. */
int repne_scas (code_ptr p,
                const QwordPtr_ES_RDI (&   unused));
/* F2 RexW AF        | Find RAX, starting at [RDI]. */
int repne_scas (code_ptr p,
...
int rep_ins (code_ptr p,
             const BytePtr_ES_EDI (&   unused1),
             const RegDX (&   unused2));
/* F3 RexW 6C        | Input RCX bytes from port DX into [RDI]. */
int rexw_rep_ins (code_ptr p,
                  const BytePtr_ES_RDI (&   unused1),
                  const RegDX (&   unused2));
/* F3 RexW 6C        | Input RCX bytes from port DX into [RDI]. */
int rexw_rep_ins (code_ptr p,
...
int rep_ins (code_ptr p,
             const DwordPtr_ES_EDI (&   unused1),
             const RegDX (&   unused2));
/* F3 RexW 6D        | Input RCX default size from port DX into [RDI]. */
int rexw_rep_ins (code_ptr p,
                  const DwordPtr_ES_RDI (&   unused1),
                  const RegDX (&   unused2));
/* F3 RexW 6D        | Input RCX default size from port DX into [RDI]. */
...
int rep_lods (code_ptr p,
              const RegAL (&   unused),
              const BytePtr_ESI (&   ptr));
/* F3 RexW AC        | Load RCX bytes from [RSI] to AL. */
int rexw_rep_lods (code_ptr p,
                   const BytePtr_RSI (&   ptr));
/* F3 RexW AC        | Load RCX bytes from [RSI] to AL. */
int rexw_rep_lods (code_ptr p,
                   const RegAL (&   unused),
...
int rep_lods (code_ptr p,
              const RegEAX (&   unused),
              const DwordPtr_ESI (&   ptr));
/* F3 RexW AD        | Load RCX quadwords from [RSI] to RAX. */
int rep_lods (code_ptr p,
              const QwordPtr_RSI (&   ptr));
/* F3 RexW AD        | Load RCX quadwords from [RSI] to RAX. */
int rep_lods (code_ptr p,
...
int rep_movs (code_ptr p,
              const BytePtr_ES_EDI (&   unused),
              const BytePtr_ESI (&   ptr));
/* F3 RexW A4        | Move RCX bytes from [RSI] to [RDI]. */
int rexw_rep_movs (code_ptr p,
                   const BytePtr_ES_RDI (&   unused),
                   const BytePtr_RSI (&   ptr));
/* F3 RexW A4        | Move RCX bytes from [RSI] to [RDI]. */
int rexw_rep_movs (code_ptr p,
...
int rep_movs (code_ptr p,
              const DwordPtr_ES_EDI (&   unused),
              const DwordPtr_ESI (&   ptr));
/* F3 RexW A5        | Move RCX quadwords from [RSI] to [RDI]. */
int rep_movs (code_ptr p,
              const QwordPtr_ES_RDI (&   unused),
              const QwordPtr_RSI (&   ptr));
/* F3 RexW A5        | Move RCX quadwords from [RSI] to [RDI]. */
...
int rep_outs (code_ptr p,
              const RegDX (&   unused),
              const BytePtr_ESI (&   ptr));
/* F3 RexW 6E        | Output RCX bytes from [RSI] to port DX. */
int rexw_rep_outs (code_ptr p,
                   const RegDX (&   unused),
                   const BytePtr_RSI (&   ptr));
/* F3 RexW 6E        | Output RCX bytes from [RSI] to port DX. */
int rexw_rep_outs (code_ptr p,
...
int rep_outs (code_ptr p,
              const RegDX (&   unused),
              const DwordPtr_ESI (&   ptr));
/* F3 RexW 6F        | Output RCX default size from [RSI] to port DX. */
int rexw_rep_outs (code_ptr p,
                   const RegDX (&   unused),
                   const DwordPtr_RSI (&   ptr));
/* F3 RexW 6F        | Output RCX default size from [RSI] to port DX. */
...
int rep_stos (code_ptr p,
              const BytePtr_ES_EDI (&   unused1),
              const RegAL (&   unused2));
/* F3 RexW AA        | Fill RCX bytes at [RDI] with AL. */
int rexw_rep_stos (code_ptr p,
                   const BytePtr_ES_RDI (&   unused));
/* F3 RexW AA        | Fill RCX bytes at [RDI] with AL. */
int rexw_rep_stos (code_ptr p,
                   const BytePtr_ES_RDI (&   unused1),
...
int rep_stos (code_ptr p,
              const DwordPtr_ES_EDI (&   unused1),
              const RegEAX (&   unused2));
/* F3 RexW AB        | Fill RCX quadwords at [RDI] with RAX. */
int rep_stos (code_ptr p,
              const QwordPtr_ES_RDI (&   unused));
/* F3 RexW AB        | Fill RCX quadwords at [RDI] with RAX. */
int rep_stos (code_ptr p,
              const QwordPtr_ES_RDI (&   unused1),
              const RegRAX (&   unused2));
/* F3 RexW AB        | Fill RCX quadwords at [RDI] with RAX. */
int rep_stos (code_ptr p,
              const QwordPtr_ES_EDI (&   unused));
/* F3 RexW AB        | Fill RCX quadwords at [RDI] with RAX. */
int rep_stos (code_ptr p,
              const QwordPtr_ES_EDI (&   unused1),
              const RegRAX (&   unused2));
...
int retf (code_ptr p,
          imm16_t imm);
/* RexW C1  /0 RMBoth  ib   | Rotate 64 bits r/m64 left imm8 times. Uses a 6 bit count. */
int rol (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /0 RMBoth  ib   | Rotate 64 bits r/m64 left imm8 times. Uses a 6 bit count. */
...
int rol (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /0 RMBoth  ib   | Rotate 8 bits r/m8 left imm8 times. */
int rol (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /0 RMBoth  ib   | Rotate 8 bits r/m8 left imm8 times. */
...
int rol (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /0 RMBoth     | Rotate 8 bits r/m8 left CL times. */
int rol (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /0 RMBoth     | Rotate 8 bits r/m8 left CL times. */
...
int rol (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /0 RMBoth     | Rotate 64 bits r/m64 left CL times. Uses a 6 bit count. */
int rol (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /0 RMBoth     | Rotate 64 bits r/m64 left CL times. Uses a 6 bit count. */
int rol (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW C1  /1 RMBoth  ib   | Rotate 64 bits r/m64 right imm8 times. Uses a 6 bit count. */
int ror (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /1 RMBoth  ib   | Rotate 64 bits r/m64 right imm8 times. Uses a 6 bit count. */
...
int ror (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /1 RMBoth  ib   | Rotate 8 bits r/m16 right imm8 times. */
int ror (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /1 RMBoth  ib   | Rotate 8 bits r/m16 right imm8 times. */
...
int ror (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /1 RMBoth     | Rotate 8 bits r/m8 right CL times. */
int ror (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /1 RMBoth     | Rotate 8 bits r/m8 right CL times. */
...
int ror (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /1 RMBoth     | Rotate 64 bits r/m64 right CL times. Uses a 6 bit count. */
int ror (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /1 RMBoth     | Rotate 64 bits r/m64 right CL times. Uses a 6 bit count. */
...
int rsqrtss (code_ptr p,
             const XmmReg (&   modrm_reg),
             const DwordPtr (&   modrm_rm));
/* RexW C1  /4 RMBoth  ib   | Multiply r/m64 by 2, imm8 times. */
int sal (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /4 RMBoth  ib   | Multiply r/m64 by 2, imm8 times. */
...
int sal (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /4 RMBoth  ib   | Multiply r/m8 by 2, imm8 times. */
int sal (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /4 RMBoth  ib   | Multiply r/m8 by 2, imm8 times. */
...
int sal (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /4 RMBoth     | Multiply r/m8 by 2, CL times. */
int sal (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /4 RMBoth     | Multiply r/m8 by 2, CL times. */
...
int sal (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /4 RMBoth     | Multiply r/m64 by 2, CL times. */
int sal (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /4 RMBoth     | Multiply r/m64 by 2, CL times. */
int sal (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW C1  /7 RMBoth  ib   | Signed divide* r/m64 by 2, imm8 times */
int sar (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /7 RMBoth  ib   | Signed divide* r/m64 by 2, imm8 times */
...
int sar (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /7 RMBoth  ib   | Signed divide* r/m8 by 2, imm8 times. */
int sar (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /7 RMBoth  ib   | Signed divide* r/m8 by 2, imm8 times. */
...
int sar (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /7 RMBoth     | Signed divide* r/m8 by 2, CL times. */
int sar (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /7 RMBoth     | Signed divide* r/m8 by 2, CL times. */
...
int sar (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /7 RMBoth     | Signed divide* r/m64 by 2, CL times. */
int sar (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /7 RMBoth     | Signed divide* r/m64 by 2, CL times. */
int sar (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW 1D     id   | Subtract with borrow sign- extended imm.32 to 64-bits from RAX. */
int sbb (code_ptr p,
         const RegRAX (&   unused),
         imm32_t imm);
/* RexW 83  /3 RMBoth  ib   | Subtract with borrow sign- extended imm8 from r/m64. */
int sbb (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm8_t imm);
/* RexW 81  /3 RMBoth  id   | Subtract with borrow sign- extended imm32 to 64-bits from r/m64. */
int sbb (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm32_t imm);
/* RexW 81  /3 RMBoth  id   | Subtract with borrow sign- extended imm32 to 64-bits from r/m64. */
...
int sbb (code_ptr p,
         const WordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 1B  /r RMBoth     | Subtract with borrow r/m64 from r64. */
int sbb (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 1B  /r RMBoth     | Subtract with borrow r/m64 from r64. */
int sbb (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordPtr (&   modrm_rm));
/* RexW 19  /r RMBoth     | Subtract with borrow r64 from r/m64. */
int sbb (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const QwordReg (&   modrm_reg));
...
int sbb (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const BytePtr86 (&   modrm_rm));
/* Rex 1A  /r RMBoth     | Subtract with borrow r/m8 from r8. */
int sbb (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
/* Rex 1A  /r RMBoth     | Subtract with borrow r/m8 from r8. */
...
int sbb (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /3 RMBoth  ib   | Subtract with borrow imm8 from r/m8. */
int sbb (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /3 RMBoth  ib   | Subtract with borrow imm8 from r/m8. */
int sbb (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         imm8_t imm);
/* Rex 18  /r RMBoth     | Subtract with borrow r8 from r/m8. */
int sbb (code_ptr p,
         const BytePtr (&   modrm_rm),
         const ByteRegRex (&   modrm_reg));
...
int scas (code_ptr p,
          const RegEAX (&   unused1),
          const DwordPtr_ES_EDI (&   unused2));
/* RexW AF        | Compare RAX with quadword at RDI or EDI then set status flags. */
int scas (code_ptr p,
          const QwordPtr_ES_RDI (&   unused));
/* RexW AF        | Compare RAX with quadword at RDI or EDI then set status flags. */
int scas (code_ptr p,
...
int seta (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 97  /r RMBoth     | Set byte if above (CF=0 and ZF=0). */
int seta (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 97  /r RMBoth     | Set byte if above (CF=0 and ZF=0). */
int seta (code_ptr p,
...
int setae (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 93  /r RMBoth     | Set byte if above or equal (CF=0). */
int setae (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 93  /r RMBoth     | Set byte if above or equal (CF=0). */
int setae (code_ptr p,
...
int setb (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 92  /r RMBoth     | Set byte if below (CF=1). */
int setb (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 92  /r RMBoth     | Set byte if below (CF=1). */
int setb (code_ptr p,
...
int setbe (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 96  /r RMBoth     | Set byte if below or equal (CF=1 or ZF=1). */
int setbe (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 96  /r RMBoth     | Set byte if below or equal (CF=1 or ZF=1). */
int setbe (code_ptr p,
...
int setc (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 92  /r RMBoth     | Set byte if carry (CF=1). */
int setc (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 92  /r RMBoth     | Set byte if carry (CF=1). */
int setc (code_ptr p,
...
int sete (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 94  /r RMBoth     | Set byte if equal (ZF=1). */
int sete (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 94  /r RMBoth     | Set byte if equal (ZF=1). */
int sete (code_ptr p,
...
int setg (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 9F  /r RMBoth     | Set byte if greater (ZF=0 and SF=OF). */
int setg (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 9F  /r RMBoth     | Set byte if greater (ZF=0 and SF=OF). */
int setg (code_ptr p,
...
int setge (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9D  /r RMBoth     | Set byte if greater or equal (SF=OF). */
int setge (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9D  /r RMBoth     | Set byte if greater or equal (SF=OF). */
int setge (code_ptr p,
...
int setl (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 9C  /r RMBoth     | Set byte if less (SF≠ OF). */
int setl (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 9C  /r RMBoth     | Set byte if less (SF≠ OF). */
int setl (code_ptr p,
...
int setle (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9E  /r RMBoth     | Set byte if less or equal (ZF=1 or SF≠ OF). */
int setle (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9E  /r RMBoth     | Set byte if less or equal (ZF=1 or SF≠ OF). */
int setle (code_ptr p,
...
int setna (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 96  /r RMBoth     | Set byte if not above (CF=1 or ZF=1). */
int setna (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 96  /r RMBoth     | Set byte if not above (CF=1 or ZF=1). */
int setna (code_ptr p,
...
int setnae (code_ptr p,
            const BytePtr86 (&   modrm_rm));
/* Rex 0F 92  /r RMBoth     | Set byte if not above or equal (CF=1). */
int setnae (code_ptr p,
            const ByteRegRex (&   modrm_rm));
/* Rex 0F 92  /r RMBoth     | Set byte if not above or equal (CF=1). */
int setnae (code_ptr p,
...
int setnb (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 93  /r RMBoth     | Set byte if not below (CF=0). */
int setnb (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 93  /r RMBoth     | Set byte if not below (CF=0). */
int setnb (code_ptr p,
...
int setnbe (code_ptr p,
            const BytePtr86 (&   modrm_rm));
/* Rex 0F 97  /r RMBoth     | Set byte if not below or equal (CF=0 and ZF=0). */
int setnbe (code_ptr p,
            const ByteRegRex (&   modrm_rm));
/* Rex 0F 97  /r RMBoth     | Set byte if not below or equal (CF=0 and ZF=0). */
int setnbe (code_ptr p,
...
int setnc (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 93  /r RMBoth     | Set byte if not carry (CF=0). */
int setnc (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 93  /r RMBoth     | Set byte if not carry (CF=0). */
int setnc (code_ptr p,
...
int setne (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 95  /r RMBoth     | Set byte if not equal (ZF=0). */
int setne (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 95  /r RMBoth     | Set byte if not equal (ZF=0). */
int setne (code_ptr p,
...
int setng (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9E  /r RMBoth     | Set byte if not greater (ZF=1 or SF≠ OF). */
int setng (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9E  /r RMBoth     | Set byte if not greater (ZF=1 or SF≠ OF). */
int setng (code_ptr p,
...
int setnge (code_ptr p,
            const BytePtr86 (&   modrm_rm));
/* Rex 0F 9C  /r RMBoth     | Set byte if not greater or equal (SF≠ OF). */
int setnge (code_ptr p,
            const ByteRegRex (&   modrm_rm));
/* Rex 0F 9C  /r RMBoth     | Set byte if not greater or equal (SF≠ OF). */
int setnge (code_ptr p,
...
int setnl (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9D  /r RMBoth     | Set byte if not less (SF=OF). */
int setnl (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9D  /r RMBoth     | Set byte if not less (SF=OF). */
int setnl (code_ptr p,
...
int setnle (code_ptr p,
            const BytePtr86 (&   modrm_rm));
/* Rex 0F 9F  /r RMBoth     | Set byte if not less or equal (ZF=0 and SF=OF). */
int setnle (code_ptr p,
            const ByteRegRex (&   modrm_rm));
/* Rex 0F 9F  /r RMBoth     | Set byte if not less or equal (ZF=0 and SF=OF). */
int setnle (code_ptr p,
...
int setno (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 91  /r RMBoth     | Set byte if not overflow (OF=0). */
int setno (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 91  /r RMBoth     | Set byte if not overflow (OF=0). */
int setno (code_ptr p,
...
int setnp (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9B  /r RMBoth     | Set byte if not parity (PF=0). */
int setnp (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9B  /r RMBoth     | Set byte if not parity (PF=0). */
int setnp (code_ptr p,
...
int setns (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 99  /r RMBoth     | Set byte if not sign (SF=0). */
int setns (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 99  /r RMBoth     | Set byte if not sign (SF=0). */
int setns (code_ptr p,
...
int setnz (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 95  /r RMBoth     | Set byte if not zero (ZF=0). */
int setnz (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 95  /r RMBoth     | Set byte if not zero (ZF=0). */
int setnz (code_ptr p,
...
int seto (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 90  /r RMBoth     | Set byte if overflow (OF=1). */
int seto (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 90  /r RMBoth     | Set byte if overflow (OF=1). */
int seto (code_ptr p,
...
int setp (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 9A  /r RMBoth     | Set byte if parity (PF=1). */
int setp (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 9A  /r RMBoth     | Set byte if parity (PF=1). */
int setp (code_ptr p,
...
int setpe (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9A  /r RMBoth     | Set byte if parity even (PF=1). */
int setpe (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9A  /r RMBoth     | Set byte if parity even (PF=1). */
int setpe (code_ptr p,
...
int setpo (code_ptr p,
           const BytePtr86 (&   modrm_rm));
/* Rex 0F 9B  /r RMBoth     | Set byte if parity odd (PF=0). */
int setpo (code_ptr p,
           const ByteRegRex (&   modrm_rm));
/* Rex 0F 9B  /r RMBoth     | Set byte if parity odd (PF=0). */
int setpo (code_ptr p,
...
int sets (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 98  /r RMBoth     | Set byte if sign (SF=1). */
int sets (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 98  /r RMBoth     | Set byte if sign (SF=1). */
int sets (code_ptr p,
...
int setz (code_ptr p,
          const BytePtr86 (&   modrm_rm));
/* Rex 0F 94  /r RMBoth     | Set byte if zero (ZF=1). */
int setz (code_ptr p,
          const ByteRegRex (&   modrm_rm));
/* Rex 0F 94  /r RMBoth     | Set byte if zero (ZF=1). */
int setz (code_ptr p,
...
int shld (code_ptr p,
          ...
          const DwordReg (&   modrm_reg),
          const RegCL (&   unused));
/* RexW 0F A4  /r RMBoth  ib   | Shift r/m64 to left imm8 places while shifting bits from r64 in from the right. */
int shld (code_ptr p,
          const QwordReg (&   modrm_rm),
          const QwordReg (&   modrm_reg),
          imm8_t imm);
...
int shld (code_ptr p,
          ...
          const QwordReg (&   modrm_reg),
          imm8_t imm);
/* RexW 0F A5  /r RMBoth     | Shift r/m64 to left CL places while shifting bits from r64 in from the right. */
int shld (code_ptr p,
          const QwordReg (&   modrm_rm),
          const QwordReg (&   modrm_reg),
          const RegCL (&   unused));
...
int shld (code_ptr p,
          ...
          const QwordReg (&   modrm_reg),
          const RegCL (&   unused));
/* RexW C1  /5 RMBoth  ib   | Unsigned divide r/m64 by 2, imm8 times. */
int shr (code_ptr p,
         const QwordReg (&   modrm_rm),
         imm8_t imm);
/* RexW C1  /5 RMBoth  ib   | Unsigned divide r/m64 by 2, imm8 times. */
...
int shr (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /5 RMBoth  ib   | Unsigned divide r/m8 by 2, imm8 times. */
int shr (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex C0  /5 RMBoth  ib   | Unsigned divide r/m8 by 2, imm8 times. */
...
int shr (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /5 RMBoth     | Unsigned divide r/m8 by 2, CL times. */
int shr (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         const RegCL (&   unused));
/* Rex D2  /5 RMBoth     | Unsigned divide r/m8 by 2, CL times. */
...
int shr (code_ptr p,
         const DwordPtr (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /5 RMBoth     | Unsigned divide r/m64 by 2, CL times. */
int shr (code_ptr p,
         const QwordReg (&   modrm_rm),
         const RegCL (&   unused));
/* RexW D3  /5 RMBoth     | Unsigned divide r/m64 by 2, CL times. */
...
int shrd (code_ptr p,
          ...
          const DwordReg (&   modrm_reg),
          const RegCL (&   unused));
/* RexW 0F AC  /r RMBoth  ib   | Shift r/m64 to right imm8 places while shifting bits from r64 in from the left. */
int shrd (code_ptr p,
          const QwordReg (&   modrm_rm),
          const QwordReg (&   modrm_reg),
          imm8_t imm);
...
int shrd (code_ptr p,
          ...
          const QwordReg (&   modrm_reg),
          imm8_t imm);
/* RexW 0F AD  /r RMBoth     | Shift r/m64 to right CL places while shifting bits from r64 in from the left. */
int shrd (code_ptr p,
          const QwordReg (&   modrm_rm),
          const QwordReg (&   modrm_reg),
          const RegCL (&   unused));
...
int sldt (code_ptr p,
          const WordPtr (&   modrm_rm));
/* RexW 0F 00  /0 RMBoth     | Stores segment selector from LDTR in r64/m16. */
int sldt (code_ptr p,
          const QwordReg (&   modrm_rm));
...
int smsw (code_ptr p,
          const DwordReg (&   modrm_rm));
/* RexW 0F 01  /4 RMBoth     | Store machine status word in low-order 16 bits of r64/m16; high-order 16 bits of r32 are undefined. */
int smsw (code_ptr p,
          const QwordReg (&   modrm_rm));
...
int stos (code_ptr p,
          const DwordPtr_ES_EDI (&   unused1),
          const RegEAX (&   unused2));
/* RexW AB        | Store RAX at address RDI or EDI. */
int stos (code_ptr p,
          const QwordPtr_ES_RDI (&   unused));
/* RexW AB        | Store RAX at address RDI or EDI. */
int stos (code_ptr p,
...
int str (code_ptr p,
         const WordPtr (&   modrm_rm));
/* RexW 2D     id   | Subtract imm32 sign- extended to 64-bits from RAX. */
int sub (code_ptr p,
         const RegRAX (&   unused),
         imm32_t imm);
/* RexW 83  /5 RMBoth  ib   | Subtract sign-extended imm8 from r/m64. */
int sub (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm8_t imm);
/* RexW 81  /5 RMBoth  id   | Subtract imm32 sign- extended to 64-bits from r/m64. */
int sub (code_ptr p,
         const QwordReg_m_RAX (&   modrm_rm),
         imm32_t imm);
/* RexW 81  /5 RMBoth  id   | Subtract imm32 sign- extended to 64-bits from r/m64. */
...
int sub (code_ptr p,
         const WordPtr (&   modrm_rm),
         imm8_t imm);
/* RexW 2B  /r RMBoth     | Subtract r/m64 from r64. */
int sub (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordReg (&   modrm_rm));
/* RexW 2B  /r RMBoth     | Subtract r/m64 from r64. */
int sub (code_ptr p,
         const QwordReg (&   modrm_reg),
         const QwordPtr (&   modrm_rm));
/* RexW 29  /r RMBoth     | Subtract r64 from r/m64. */
int sub (code_ptr p,
         const QwordPtr (&   modrm_rm),
         const QwordReg (&   modrm_reg));
...
int sub (code_ptr p,
         const ByteReg86 (&   modrm_reg),
         const BytePtr86 (&   modrm_rm));
/* Rex 2A  /r RMBoth     | Subtract r/m8 from r8. */
int sub (code_ptr p,
         const ByteRegLow (&   modrm_reg),
         const ByteRegRex (&   modrm_rm));
/* Rex 2A  /r RMBoth     | Subtract r/m8 from r8. */
...
int sub (code_ptr p,
         const BytePtr86 (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /5 RMBoth  ib   | Subtract imm8 from r/m8. */
int sub (code_ptr p,
         const ByteRegRex (&   modrm_rm),
         imm8_t imm);
/* Rex 80  /5 RMBoth  ib   | Subtract imm8 from r/m8. */
int sub (code_ptr p,
         const BytePtrRex (&   modrm_rm),
         imm8_t imm);
/* Rex 28  /r RMBoth     | Subtract r8 from r/m8. */
int sub (code_ptr p,
         const BytePtr (&   modrm_rm),
         const ByteRegRex (&   modrm_reg));
...
int subss (code_ptr p,
           const XmmReg (&   modrm_reg),
           const DwordPtr (&   modrm_rm));
/*  0F 01 F8        | Exchanges the current GS base register value with the value contained in MSR address C0000102H. */
int swapgs (code_ptr p);
/*  0F 05        | Fast call to privilege level 0 system procedures. */
int syscall (code_ptr p);
...
/* RexW 0F 35        | Fast return to 64-bit mode privilege level 3 user code. */
int rexw_sysexit (code_ptr p);
/*  0F 07        | Return from fast system call */
int sysret (code_ptr p);
/*  84  /r RMBoth     | AND r8 with r/m8; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const ByteReg86 (&   modrm_rm),
          const ByteReg86 (&   modrm_reg));
/* Rex 84  /r RMBoth     | AND r8 with r/m8; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const ByteRegLow (&   modrm_rm),
          const ByteRegRex (&   modrm_reg));
/* Rex 84  /r RMBoth     | AND r8 with r/m8; set SF, ZF, PF according to result. */
...
int test (code_ptr p,
          const RegEAX (&   unused),
          imm32_t imm);
/* RexW A9     id   | AND imm32 sign-extended to 64-bits with RAX; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const RegRAX (&   unused),
          imm32_t imm);
...
int test (code_ptr p,
          const BytePtr86 (&   modrm_rm),
          imm8_t imm);
/* Rex F6  /0 RMBoth  ib   | AND imm8 with r/m8; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const ByteRegRex (&   modrm_rm),
          imm8_t imm);
/* Rex F6  /0 RMBoth  ib   | AND imm8 with r/m8; set SF, ZF, PF according to result. */
...
int test (code_ptr p,
          const DwordPtr (&   modrm_rm),
          const DwordReg (&   modrm_reg));
/* RexW F7  /0 RMBoth  id   | AND imm32 sign-extended to 64-bits with r/m64; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const QwordReg (&   modrm_rm),
          imm32_t imm);
/* RexW F7  /0 RMBoth  id   | AND imm32 sign-extended to 64-bits with r/m64; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const QwordPtr (&   modrm_rm),
          imm32_t imm);
/* RexW 85  /r RMBoth     | AND r64 with r/m64; set SF, ZF, PF according to result. */
int test (code_ptr p,
          const QwordReg (&   modrm_rm),
          const QwordReg (&   modrm_reg));
/* RexW 85  /r RMBoth     | AND r64 with r/m64; set SF, ZF, PF according to result. */
...
int vmptrst (code_ptr p,
             const QwordPtr (&   modrm_rm));
/*  0F 78  /r RMBoth     | Reads a specified VMCS field (in 64-bit mode). */
...
/*  0F 79  /r RMBoth     | Writes a specified VMCS field (in 64-bit mode) */
...
int xadd (code_ptr p,
          const ByteReg86 (&   modrm_rm),
          const ByteReg86 (&   modrm_reg));
/* Rex 0F C0  /r RMBoth     | Exchange r8 and r/m8; load sum into r/m8. */
int xadd (code_ptr p,
          const ByteRegLow (&   modrm_rm),
          const ByteRegRex (&   modrm_reg));
/* Rex 0F C0  /r RMBoth     | Exchange r8 and r/m8; load sum into r/m8. */
...
int xadd (code_ptr p,
          const DwordPtr (&   modrm_rm),
          const DwordReg (&   modrm_reg));
/* RexW 0F C1  /r RMBoth     | Exchange r64 and r/m64; load sum into r/m64. */
int xadd (code_ptr p,
          const QwordReg (&   modrm_rm),
          const QwordReg (&   modrm_reg));
/* RexW 0F C1  /r RMBoth     | Exchange r64 and r/m64; load sum into r/m64. */
...
int xadd (code_ptr p,
          const BytePtrRex (&   modrm_rm),
          const ByteRegLow (&   modrm_reg));
/* RexW 87  /r RMBoth     | Exchange r64 with quadword from r/m64. */
int xchg (code_ptr p,
          const RegRAX (&   modrm_rm),
          const QwordReg (&   modrm_reg));
/* RexW 87  /r RMBoth     | Exchange r64 with quadword from r/m64. */
int xchg (code_ptr p,
          const QwordReg_m_RAX (&   modrm_rm),
          const QwordReg (&   modrm_reg));
/* RexW 90    +rd    | Exchange RAX with r64. */
int xchg (code_ptr p,
          const QwordReg_m_RAX (&   radd),
          const RegRAX (&   unused));
/* RexW 87  /r RMBoth     | Exchange r64 with quadword from r/m64. */
int xchg (code_ptr p,
          const QwordPtr (&   modrm_rm),
          const QwordReg (&   modrm_reg));
/* RexW 87  /r RMBoth     | Exchange quadword from r/m64 with r64. */
int xchg (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordPtr (&   modrm_rm));
...
int xchg (code_ptr p,
          const ByteReg86 (&   modrm_reg),
          const BytePtr86 (&   modrm_rm));
/* Rex 86  /r RMBoth     | Exchange byte from r/m8 with r8 (byte register). */
int xchg (code_ptr p,
          const ByteRegLow (&   modrm_reg),
          const ByteRegRex (&   modrm_rm));
/* Rex 86  /r RMBoth     | Exchange byte from r/m8 with r8 (byte register). */
...
int xchg (code_ptr p,
          const ByteRegRex (&   modrm_reg),
          const BytePtr (&   modrm_rm));
/* Rex 86  /r RMBoth     | Exchange r8 (byte register) with byte from r/m8. */
int xchg (code_ptr p,
          const BytePtr (&   modrm_rm),
          const ByteRegRex (&   modrm_reg));
...
int xlat (code_ptr p,
          const BytePtr_RBX (&   ptr));
/*  D7        | Set AL to memory byte DS:[(E)BX + unsigned AL]. */
int xlat (code_ptr p,
          const BytePtr_EBX (&   ptr));
/*  D7        | Set AL to memory byte DS:[(E)BX + unsigned AL]. */
int xlatb (code_ptr p);
/* RexW D7        | Set AL to memory byte [RBX + unsigned AL]. */
int rexw_xlatb (code_ptr p);
/* RexW 35     id   | RAX XOR imm32 (sign- extended). */
int xor_ (code_ptr p,
          const RegRAX (&   unused),
          imm32_t imm);
/* RexW 83  /6 RMBoth  ib   | r/m64 XOR imm8 (sign- extended). */
int xor_ (code_ptr p,
          const QwordReg_m_RAX (&   modrm_rm),
          imm8_t imm);
/* RexW 81  /6 RMBoth  id   | r/m64 XOR imm32 (sign- extended). */
int xor_ (code_ptr p,
          const QwordReg_m_RAX (&   modrm_rm),
          imm32_t imm);
/* RexW 81  /6 RMBoth  id   | r/m64 XOR imm32 (sign- extended). */
...
int xor_ (code_ptr p,
          const WordPtr (&   modrm_rm),
          imm8_t imm);
/* RexW 33  /r RMBoth     | r64 XOR r/m64. */
int xor_ (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordReg (&   modrm_rm));
/* RexW 33  /r RMBoth     | r64 XOR r/m64. */
int xor_ (code_ptr p,
          const QwordReg (&   modrm_reg),
          const QwordPtr (&   modrm_rm));
/* RexW 31  /r RMBoth     | r/m64 XOR r64. */
int xor_ (code_ptr p,
          const QwordPtr (&   modrm_rm),
          const QwordReg (&   modrm_reg));
...
int xor_ (code_ptr p,
          const BytePtr86 (&   modrm_rm),
          imm8_t imm);
/* Rex 80  /6 RMBoth  ib   | r/m8 XOR imm8. */
int xor_ (code_ptr p,
          const ByteRegRex (&   modrm_rm),
          imm8_t imm);
/* Rex 80  /6 RMBoth  ib   | r/m8 XOR imm8. */
...
int xrstor (code_ptr p,
            const VoidPtr (&   modrm_rm));
/* RexW 0F AE  /5 RMMemOnly     | Restore processor extended states from memory. The states are specified by EDX:EAX */
int xrstor64 (code_ptr p,
              const VoidPtr (&   modrm_rm));
/*  0F AE  /4 RMMemOnly     | Save processor extended states to memory. The states are specified by EDX:EAX */
int xsave (code_ptr p,
           const VoidPtr (&   modrm_rm));
/* RexW 0F AE  /4 RMMemOnly     | Save processor extended states to memory. The states are specified by EDX:EAX */
int xsave64 (code_ptr p,
             const VoidPtr (&   modrm_rm));
